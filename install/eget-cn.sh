...
#
# Acknowledgments:
#   - getmic.ro: https://githubfast.com/benweissmann/getmic.ro

set -e -u

githubLatestTag() {
  finalUrl=$(curl "https://githubfast.com/$1/releases/latest" -s -L -I -o /dev/null -w '%{url_effective}')
  printf "%s\n" "${finalUrl##*v}"
}
...
printf "Downloading https://githubfast.com/zyedidia/eget/releases/download/v%s/eget-%s-%s.%s\n" "$TAG" "$TAG" "$platform" "$extension"

curl -L "https://githubfast.com/zyedidia/eget/releases/download/v$TAG/eget-$TAG-$platform.$extension" > "eget.$extension"
