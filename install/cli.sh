...
retry -- curl https://getcroc.schollz.com | retry -- bash -x
...
retry -- curl --max-time 60 https://raw.githubusercontent.com/jesseduffield/lazydocker/master/scripts/install_update_linux.sh | DIR=/usr/local/bin sudo retry -- bash -x
